...
def autocorrelation2d(stack, offset, offset_scale=1, center=True):
    ...
    """
    res = autocorrelation_image(stack.squeeze(),offset,offset_scale,center)
    if stack.dim()==4:
...
# Given a 2d image and an offset, return the product of the image and a offset/shifted version of itself
def autocorrelation_image(image, offset, offset_scale=1, center=True):
    ...
    if image.dim() != 2: raise ValueError(f'Expected 2 dimensional image but got: {image.size()}')
...
def offsetcorrelation2d(imageA,imageB, offset, offset_scale=1, center=True):
    ...
    """
    res = offsetcorrelation_image(imageA.squeeze(),imageB.squeeze(),offset,offset_scale,center)
    if imageA.dim()==4:
        return res.unsqueeze(0).unsqueeze(0)
    elif imageA.dim()==3:
        return res.unsqueeze(0)
...
def offsetcorrelation_image(imageA, imageB, offset, offset_scale=1, center=True):
    ...
    if imageA.dim() != 2: raise ValueError(f'Expected 2 dimensional image but got: {imageA.size()}')
